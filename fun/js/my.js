...
function timezone(){
    ...
    var birthday=convertTimes("2015-10-28",16,58,0);
    var distance=birthday-Date.parse(myDate);
    if(distance>0){
        alert("时间还没到");
    }else{
        var result=0;
        if(Date.parse(myDate)>convertTimes("2015-10-28",18,0,0)){
            result=7;
        }else if(hour<8){
        ...
        switch (result){
            ...
            case 5:
                $("#timezone").find("button:eq(5) a").attr("href","../8text-pixel");
...
function jisuanBirthday(){
    ...
    if( parseInt(birthday.split("-")[0])>1999){
